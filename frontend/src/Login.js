...
// // פונקציה לטיפול בשגיאות מהבקאנד
// const handleBackendErrors = (data) => {
//   let errorMessages = [];
  
//   if (data.errors) {
//     // אם יש מערך של שגיאות
//     if (Array.isArray(data.errors)) {
//       errorMessages = data.errors;
//     } else if (typeof data.errors === 'object') {
//       // אם השגיאות הן אובייקט עם שדות
...
//     // שגיאות שלא קשורות לשדה ספציפי
//     if (Array.isArray(data.non_field_errors)) {
//       errorMessages = [...errorMessages, ...data.non_field_errors];
//     }
//   }
  
//   return errorMessages.length > 0 ? errorMessages : ['שגיאה לא ידועה'];
// };
...
//     // Custom validation - replace React's automatic validation
//     const validationErrors = [];
    
//     if (!email || email.trim() === '') {
//       validationErrors.push('נא להזין כתובת אימייל');
//     } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
//       validationErrors.push('כתובת האימייל אינה תקינה');
//     }
    
//     if (!password || password.trim() === '') {
//       validationErrors.push('נא להזין סיסמה');
//     }
...
//         setErrors(backendErrors.length > 0 ? backendErrors : ['אימייל או סיסמה שגויים']);
...
//       } else if (response.status === 404) {
//         // User not found, redirect to registration
//         setErrors(['משתמש לא נמצא. האם תרצה להירשם?']);
//         // אופציונלי: לא לעשות redirect אוטומטי
//         // navigate('/register');
//       } else if (response.status >= 500) {
//         setErrors(['שגיאת שרת פנימית. נסה שוב מאוחר יותר.']);
//       } else {
//         const backendErrors = handleBackendErrors(data);
//         setErrors(backendErrors);
//       }
...
//       setErrors(['שגיאת רשת. בדוק את החיבור לאינטרנט ונסה שוב.']);
//     } finally {
//       setIsLoading(false);
//     }
//   };
...
//   const handleForgotPassword = async () => {
//     if (!email) {
//       setErrors(['נא להזין את כתובת האימייל שלך כדי לאפס את הסיסמה']);
//       return;
//     }
...
//       if (res.ok) {
//         alert('אם קיים חשבון עם האימייל הזה, נשלח קישור איפוס סיסמה.');
//         setErrors([]);
//       } else {
//         const data = await res.json();
//         const backendErrors = handleBackendErrors(data);
