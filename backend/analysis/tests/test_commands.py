...
class LoadTermsCommandTests(TestCase):
    ...
    def test_load_drug_terms(self):
        ...
        self.assertEqual(Drug.objects.count(), 2)  # no duplicate drugs
        self.assertTrue(Drug.objects.filter(name="ASPIRIN").exists())
        self.assertTrue(Drug.objects.filter(name="IBUPROFEN").exists())
    ...
    def test_load_reaction_terms(self):
        ...
        self.assertEqual(Reaction.objects.count(), 2)  # no duplicate reactions
        self.assertTrue(Reaction.objects.filter(name="NAUSEA").exists())
        self.assertTrue(Reaction.objects.filter(name="HEADACHE").exists())
    ...
    def test_does_not_duplicate_existing_drugs_and_reactions(self):
        ...
        Drug.objects.create(name="ASPIRIN")
        Reaction.objects.create(name="NAUSEA")
